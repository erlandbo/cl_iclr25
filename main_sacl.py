...
def main_train():
    ...
    args.savedir = args.logdir + "/{current_time}_{criterion}_single_s{single_s}_{metric}_{dataset}_batch{batchsize}_epochs{epochs}_{arch}_lr{lr}_temp{temp}_alpha{alpha}_rho{rho}_s_init_t{s_init_t}_outfeats{outfeatures}".format(
